...
import cv2 # 用opencv-python替代scipy中不支持的imread/imresize
    ...
    for impaths, imcaps, split in [(train_image_paths, train_image_captions, 'TRAIN'),
        ...
        with h5py.File(os.path.join(output_folder, split + '_IMAGES_' + base_filename + '.hdf5'), 'a') as h:
            ...
            for i, path in enumerate(tqdm(impaths)):
                ...
                # Read images
                # 读取原始图像数据->可能是灰度图/BGR三通道图->灰度图转化为BGR三通道图->统一转化为RGB三通道图
                # 缩放图像大小->转置图像矩阵->断言图像维度和像素值范围
                img = cv2.imread(impaths[i], cv2.IMREAD_UNCHANGED)
                if img is None:
                    raise FileNotFoundError(f"Could not read image: {impaths[i]}")
                if img.ndim == 2:
...
def save_checkpoint(model_name, epoch, epochs_since_improvement, encoder, decoder, encoder_optimizer, decoder_optimizer,
    ...
    # copy if this checkpoint the best so far, so it will not be overwritten by a worse state
    if is_best:
        torch.save(state, os.path.join('..', 'BEST_' + filename))
...
def visualize_att(image_path, seq, alphas, rev_word_map, smooth=True):
    ...
    for t in range(len(words)):
        if t > 50:
            break
        plt.subplot(int(np.ceil(len(words) / 5.)), 5, t + 1) # 5列显示，每行显示5个子图，结果转为int类型
